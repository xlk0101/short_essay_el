...
class CRF(nn.Module):
    ...
    def __init__(self, num_tag):
        if num_tag <= 0:
            raise ValueError("Invalid value of num_tag: {}".format(num_tag))
        ...
        # P_j* 表示所有从tag_j出发的边
        # P_*k 表示所有到tag_k的边
        self.transitions = nn.Parameter(torch.Tensor(num_tag + 2, num_tag + 2)).to(comConfig.device)
    ...
    def real_path_score(self, features, tags):
        ...
        """
        r = torch.LongTensor(range(features.size(0))).to(comConfig.device)
        pad_start_tags = torch.cat([torch.LongTensor([self.start_tag]).to(comConfig.device), tags])
        pad_stop_tags = torch.cat([tags, torch.LongTensor([self.end_tag]).to(comConfig.device)])
    ...
    def all_possible_path_score(self, features):
        ...
        step1: 将forward列expand成tag_num*tag_num
        step2: 将下个单词的emission行expand成tag_num*tag_num
        ...
        for i in range(0, time_steps):
            ...
            if i == 0:
                ...
                trans_score = self.transitions[:self.start_tag, :self.start_tag]
            sum = emission_start.to(comConfig.device) + emission_end.to(comConfig.device) + trans_score.to(comConfig.device)
    ...
    def negative_log_loss(self, inputs, output_mask, tags):
        ...
        target_function = P_real_path_score/P_all_possible_path_score
                        = exp(S_real_path_score)/ sum(exp(certain_path_score)) -- softmax
        ...
        """
        inputs.to(comConfig.device)
        output_mask.to(comConfig.device)
        tags.to(comConfig.device)

        loss = Variable(torch.tensor(0.).to(comConfig.device), requires_grad=True)
    ...
    def viterbi(self, features):
        ...
        # START_TAG
        forward = Variable(torch.zeros(self.num_tag))
        forward.to(comConfig.device)
        # back_points 到该点的最大分数  last_points 前一个点的索引
        back_points, index_points = [self.transitions[self.start_tag, :self.start_tag].to(comConfig.device)], [
            torch.LongTensor([-1]).to(comConfig.device).expand_as(forward)]
        # START_TAG -> 1st word -> 2nd word ->...->END_TAG
...
def padding(vec, max_len, pad_token=-1):
    new_vec = torch.zeros(max_len).long().to(comConfig.device)
